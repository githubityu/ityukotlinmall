...
public abstract class BaseSwaggerConfig {
    ...
    public Docket createRestApi() {
        ...
        Parameter parameter = ticketPar.name(tokenHeader).description("用户信息")
                .modelRef(new ModelRef("string")).parameterType("header")
                .required(false).build();//header中的ticket参数非必填，传空也可以
//        Parameter parameter2 = ticketPar
//                .name(tokenHead).description("token")
//                .modelRef(new ModelRef("string")).parameterType("header")
//                .required(false).build();//header中的ticket参数非必填，传空也可以
        pars.add(parameter);    //根据每个方法名也知道当前方法在设置什么参数
       // pars.add(parameter2);    //根据每个方法名也知道当前方法在设置什么参数
